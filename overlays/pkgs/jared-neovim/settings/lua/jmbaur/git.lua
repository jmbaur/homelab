local stdout_or_bail = function(system_call)
  ...
  if res.code > 0 then error(vim.trim(res.stderr), vim.log.levels.ERROR) end

  return vim.trim(res.stdout)
end

local get_range = function(args)
  if args.range == 0 then return { nil, nil } end

  if args.line1 == args.line2 then return { args.line1, nil } end
...
local construct_url = function(base_url_fmt, line1_fmt, line2_fmt)
  return function(args, remote_url, rev, git_file)
    ...
    if line1 ~= nil then url = url .. string.format(line1_fmt, line1) end
    if line2 ~= nil then url = url .. string.format(line2_fmt, line2) end
    return url
  end
end
...
local construct_gitea_url = function(args, remote_url, rev, git_file)
  ...
  if line1 ~= nil then url = string.format('%s#L%s', url, line1) end
  if line2 ~= nil then url = string.format('%s-L%s', url, line2) end
  return url
end
...
vim.api.nvim_create_user_command('Permalink', function(args)
  ...
  if url then
    if args.bang then vim.fn.setreg('+', url) end
    vim.print(url)
  else
