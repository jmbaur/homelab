...
local nix_repl = function() return "nix repl --expr 'import <nixpkgs> { }'" end

local shell = function() return os.getenv('SHELL') or '/bin/sh' end
...
M.run = function(cmd)
  ...
  if cmd_fn ~= nil then cmd = cmd_fn() end
...
M.launch = function() vim.ui.select(M.select_results, { prompt = 'Run:' }, M.run) end
  ...
  vim.api.nvim_create_user_command('Run', function(args)
    ...
    complete = function(arg_lead, cmd_line, cursor_pos)
      ...
      for k in pairs(M.builtins) do
        ...
        if i == 1 then table.insert(completions, k) end
      end
      return completions
    end,
  })
