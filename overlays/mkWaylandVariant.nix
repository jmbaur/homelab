...
in
drv:
if lib.hasAttr "commandLineArgs" (lib.functionArgs drv.override) then
  (drv.override {
    commandLineArgs = [
  ...
  let
    mainProgram = if lib.hasAttr "mainProgram" drv.meta then drv.meta.mainProgram else drv.pname;
  in
  symlinkJoin rec {
