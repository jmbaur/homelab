...
let
  mkInternalInterface = network:
    let
      # TODO(jared): move this to inventory file
      ipv6FourthHextet = lib.toHexString network.id;
      networkGuaPrefix = "${secrets.networking.guaPrefix}:${ipv6FourthHextet}";
      networkUlaPrefix = "${secrets.networking.ulaPrefix}:${ipv6FourthHextet}";
      guaNetwork = "${networkGuaPrefix}::/${toString network.ipv6Cidr}";
      ulaNetwork = "${networkUlaPrefix}::/${toString network.ipv6Cidr}";
    in
    {
      netdev = {
