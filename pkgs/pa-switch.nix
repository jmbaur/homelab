...
writeShellScriptBin "pa-switch" ''
  ...
  CURRENT_DEVICE=$(${pulseaudio}/bin/pacmd stat | grep "Default ''${DEVICE_TYPE} name" | awk -F ": " '{print $2}')
  DEVICES=$(${pulseaudio}/bin/pactl list "''${DEVICE_TYPE}"s | grep -e "Name: " -e "device.class")
  ...
  if [[ ''${NEXT_DEVICE} != "" ]]; then
    ...
    ${pulseaudio}/bin/pactl set-default-"''${DEVICE_TYPE}" "''${NEXT_DEVICE}"
  elif [[ ''${FIRST_DEVICE} != "" ]]; then
    echo "Setting device to ''${FIRST_DEVICE}"
    ${pulseaudio}/bin/pactl set-default-"''${DEVICE_TYPE}" "''${FIRST_DEVICE}"
  else
