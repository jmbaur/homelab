...
fn main() {
    ...
    update_repart_file(
        ...
        std::path::Path::new("/etc/repart.d/11-usr-a.conf"),
    );
    update_repart_file(
        usr_hash_size + max_usr_hash_padding,
        std::path::Path::new("/etc/repart.d/11-usr-hash-a.conf"),
    );
    update_repart_file(
        usr_size + max_usr_padding,
        std::path::Path::new("/etc/repart.d/11-usr-b.conf"),
    );
    update_repart_file(
        usr_hash_size + max_usr_hash_padding,
        std::path::Path::new("/etc/repart.d/11-usr-hash-b.conf"),
    );
}
