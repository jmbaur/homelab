{ config, pkgs, ... }:
let
  sdCardFirmware = pkgs.bpiR3Firmware.override { internalBoot = false; };
...
in
{
  assertions = [
    {
      assertion = config.hardware.bpi-r3.enable;
      message = "config.hardware.bpi-r3.enable is not true";
    }
  ];

  hardware.bpi-r3.internalBoot = false;
  ...
  system.build.sdImage = pkgs.buildPackages.callPackage
    ({ stdenv
     , dosfstools
     , e2fsprogs
     , gptfdisk
     ...
     , util-linux
      ...
      stdenv.mkDerivation {
        ...
        buildCommand = ''
          ...
          # Copy firmware
          dd conv=notrunc if=${sdCardFirmware}/bl2.img of=$img seek=$bl2Start
          dd conv=notrunc if=${sdCardFirmware}/fip.bin of=$img seek=$fipStart
