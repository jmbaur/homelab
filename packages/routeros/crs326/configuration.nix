...
let
  ...
  configuration = {
    ...
    commands = [
      ...
      "/interface bridge port add bridge=bridge frame-types=admit-only-untagged-and-priority-tagged interface=ether21 pvid=${toString inventory.networks.public.id}"
      ...
      "/interface bridge vlan add bridge=bridge untagged=ether21 vlan-ids=${toString inventory.networks.public.id}"
