...
let
  ...
  configuration = {
    ...
    commands = [
      ...
      "/ip address add address=${inventory.networks.mgmt.hosts.switch1._computed._ipv4}/${toString inventory.networks.mgmt._computed._ipv4Cidr} interface=${inventory.networks.mgmt.name} network=${inventory.networks.mgmt._computed._networkIPv4}"
      "/ip dns set servers=${inventory.networks.mgmt.hosts.artichoke._computed._ipv4}"
      "/ip route add dst-address=0.0.0.0/0 gateway=${inventory.networks.mgmt.hosts.artichoke._computed._ipv4}"
      ...
      "/ipv6 address add address=${inventory.networks.mgmt.hosts.switch1._computed._ipv6.ula} advertise=no interface=${inventory.networks.mgmt.name}"
