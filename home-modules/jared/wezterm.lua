...
-- modus vivendi adapted to fit with habamax neovim colorscheme
local modus_vivendi_modified = wezterm.color.get_builtin_schemes()["Modus-Vivendi"]
modus_vivendi_modified.background = "#1c1c1c"
modus_vivendi_modified.foreground = "#bcbcbc"
...
wezterm.on("update-status", function(window, _)
	...
	if active_key_table ~= nil then
		table.insert(status, { Text = "[" .. active_key_table .. "] " })
	end
	...
	window:perform_action(
		action.InputSelector({
			action = wezterm.action_callback(function(inner_window, inner_pane, id, label)
				...
				-- Nothing to do.
				if label == current_workspace then
					return
				end
...
local move_tab_to_last = wezterm.action_callback(function(window, pane)
	window:perform_action(action.MoveTab(#window:mux_window():tabs() - 1), pane)
end)
