...
let
  configScript = writeShellScriptBin "configScript" ''
    ...
    cat << EOF
    # jun/11/2022 16:37:55 by RouterOS 7.3.1
    # software id = TD07-GRS2
    #
    # model = RBcAPGi-5acD2nD
    /interface bridge
    add admin-mac=DC:2C:6E:0D:FE:6F auto-mac=no comment=defconf ingress-filtering=no name=bridge vlan-filtering=yes
    /interface wireless
    ...
    add bridge=bridge comment=defconf frame-types=admit-only-vlan-tagged interface=wlan2 pvid=30
    add bridge=bridge ingress-filtering=no interface=ether1
    add bridge=bridge frame-types=admit-only-vlan-tagged interface=wlan3 pvid=40
    add bridge=bridge frame-types=admit-only-vlan-tagged interface=wlan4 pvid=50
    ...
    /ip route
    add disabled=no dst-address=0.0.0.0/0 gateway=192.168.88.1 pref-src=192.168.88.4
    /ip service
